...
We then designed a **constant-velocity motion model on the SE(3) Lie group** and implemented an **adaptive Extended Kalman Filter (EKF)**. Each camera’s pose measurement is fused asynchronously as it becomes available. Further description of the system can be found in {% cite LARK %}. The current system achieves an estimated tracking error on the order of **1 mm**, comparable to the performance of traditional infrared camera-based systems.
